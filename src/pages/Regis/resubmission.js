...
const Resubmission = ({route, navigation}) => {
  ...
  React.useEffect(() => {
    // setIsLoading(true);
    setTimeout(async () => {
      setTolak(user.userable.alasan_tolak);
      setUserName(user.username);
      setPassword(user.password);
      setNamaToko(user.userable.tk_nama);
      setNamaPemilik(user.userable.tk_pemilik);
      setAlamat(user.userable.tk_alamat);
      setEmail(user.email);
      setTelp(user.userable.tk_telp);
  ...
  const submit = () => {
    ...
    setIsLoading(true);
    Axios.post(
      `https://secret-earth-93408.herokuapp.com/api/resubmission/${user.userable.id}`,
      data,
    )
      .then(res => {
        ...
        Alert.alert('Berhasil', 'Perubahan berhasil dikirim', [{text: 'OK'}]);
  ...
  return (
    <View style={styles.page}>
      <View
        ...
        <ScrollView>
          <Text
            style={{marginHorizontal: 10, marginVertical: 10, fontSize: 12}}>
            Data yang anda inputkan tidak layak.
          </Text>
          <Text
            style={{marginHorizontal: 10, fontSize: 12, fontWeight: 'bold'}}>
            Keterangan
          </Text>
          <Text style={{marginHorizontal: 15, fontSize: 12}}>{tolak}</Text>
          <Text
            style={{marginHorizontal: 10, marginVertical: 10, fontSize: 12}}>
            Silahkan mendaftar kembali dengan data yang sebenarnya.
          </Text>
        </ScrollView>
      </View>
      {message != 'success' ? (
