...
const ProductPage = ({route, navigation}) => {
    ...
    Axios.post(`https://secret-earth-93408.herokuapp.com/api/pesanan`, data, {
      ...
      .then(function (response) {
        if (response.data.mitra == undefined) {
          // console.warn(response.data.distributor);
          setMitra(false);
          setIsLoading(false);
          Alert.alert(
            'Gagal',
  ...
  const cart = async () => {
    ...
    setIsLoading(true);
    Axios.post(`https://secret-earth-93408.herokuapp.com/api/keranjang`, data, {
      headers: {
        Authorization: 'Bearer ' + token,
      },
    })
    ...
    <View style={styles.page}>
      ...
      <ScrollView style={styles.body}>
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.isi, {fontWeight: 'bold', marginBottom: 4}]}>
            {dis.ds_nama}
          </Text>
        </TouchableOpacity>
        <Text style={styles.isi}>{dis.ds_alamat}</Text>
